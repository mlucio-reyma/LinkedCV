...
## Addendum: Research on Implementation Best Practices (Requested)

### 1) i18n en Vanilla JavaScript (sin librerías)

**Best practices**:
- Usar llaves planas consistentes (por ejemplo: `nav.home`, `hero.tagline`) para evitar estructuras profundas y facilitar búsquedas.
- Cargar traducciones con `fetch()` en `DOMContentLoaded` y aplicar en una sola pasada a todos los elementos `data-i18n`.
- Persistir el idioma en `localStorage` y permitir override por URL (`?lang=en`) para enlaces compartibles.
- Definir un fallback claro: si falta una llave en ES, usar EN; si falta en ambos, mostrar la llave.
- Evitar innerHTML cuando no sea necesario; preferir `textContent` para seguridad y rendimiento.
- Agrupar texto dinámico (ej. botones, CTAs) en llaves específicas para evitar concatenaciones manuales.
- Mantener traducciones en archivos JSON estáticos (`lang/es.json`, `lang/en.json`) y validarlas con una pequeña rutina de consola que compare llaves.

**Decisión aplicada**: JSON + `data-i18n` + `localStorage` + `?lang=` (consistente con Decision 4).

### 2) Animaciones y transiciones CSS para UX suave

**Best practices**:
- Animar solo propiedades que no disparan reflow: `transform`, `opacity` (evitar `width`, `height`, `top`).
- Usar `prefers-reduced-motion` para respetar usuarios sensibles al movimiento.
- Mantener duraciones entre 200–600ms; transiciones mayores solo para hero/intro.
- Aplicar `will-change` con moderación, solo a elementos animados en scroll.
- Disparar animaciones de entrada con `IntersectionObserver` en lugar de eventos de scroll intensivos.
- Usar curvas `ease-out` para elementos que entran y `ease-in` para elementos que salen.

**Decisión aplicada**: CSS Keyframes + Intersection Observer (Decision 6).

### 3) Accesibilidad para portafolios

**Best practices**:
- Jerarquía semántica: un solo `h1`, luego `h2` por sección, `h3` para subsecciones.
- Etiquetas `aria-label` en iconos y botones sin texto visible.
- Contraste mínimo 4.5:1 y foco visible (outline de 2px).
- Navegación completa con teclado: Tab, Enter, Escape (cerrar menú móvil).
- Texto alternativo en imágenes (profile, proyectos); decorativas con `alt=""`.
- Objetivos táctiles ≥ 44px en mobile.
- Validar con Lighthouse + NVDA/VoiceOver (prueba básica).

**Decisión aplicada**: WCAG 2.1 AA (Decision 8).

### 4) Optimización de performance en sitios estáticos

**Best practices**:
- Comprimir imágenes y servir WebP con fallback JPG.
- Activar `loading="lazy"` en imágenes fuera de la vista inicial.
- Definir `width`/`height` en imágenes para evitar CLS.
- Minimizar CSS/JS (sin build tools, pero ordenar y evitar duplicidad).
- Usar fuentes del sistema (evitar carga de Google Fonts).
- Evitar dependencias externas y scripts innecesarios.
- Revisar presupuesto de tamaño total (<2MB) y usar Lighthouse para validar.

**Decisión aplicada**: WebP + lazy loading + tamaño total <2MB (Decision 5 + Constitution IV).

### 5) Buenas prácticas de despliegue en GitHub Pages

**Best practices**:
- Usar rama `main` como fuente (root `/`) para despliegue simple.
- Evitar rutas relativas rotas (usar rutas relativas desde `/` correctamente).
- Configurar `404.html` opcional para manejo de rutas (si se añade navegación avanzada).
- Verificar que `CNAME` esté configurado si se usa dominio personalizado.
- Probar el sitio publicado para garantizar que rutas de assets funcionan en el entorno GitHub Pages.
- Mantener `index.html` en raíz para compatibilidad total.

**Decisión aplicada**: GitHub Pages manual para MVP (Decision 12).
